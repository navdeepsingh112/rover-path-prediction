...
const Beetle = forwardRef(({ args = [1.7, 1, 4], mass = 500, ...props }, ref) => {
    ...
    // useFrame(() => {
    //     const { forward, backward, left, right, brake, reset , camera } = controls.current
    //     if(!isLoaded){
    //         camera ? si(i+1<pos.length?i+i:i-1) : setIsLoaded(false);
    //         console.log(i);
    //     }
    // })
    useEffect(() => {
        const handleKeyDown = (event) => {
            if (event.key === 'v') {
                // Ensure we do not exceed the length of the pos array
                si((prevI) => (prevI + 1) % pos.length);
            }
        };
    ...
    return (
        <Box ref={ref} args={[0,0,0]} castShadow>
            <primitive object={gltf.scene} rotation={[0,-Math.PI/2,0]}/>
              {i == pos.length -1 ? <OrbitControls /> : <></>}
            <PerspectiveCamera makeDefault position={pos[i][0]} rotation={pos[i][1]} />
